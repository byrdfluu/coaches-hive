...
export default function AdminOperationalDataPage({ title, description, endpoint, columns }: { title: string; description: string; endpoint: string; columns: AdminColumn[] }) {
  ...
  const load = useCallback(async () => {
    ...
    const params = new URLSearchParams(); if (query) params.set('query', query); if (status) params.set('status', status); if (workspaceId) params.set('workspace_id', workspaceId)
    ...
    setLoading(false)
  }, [endpoint, query, status, workspaceId])
  ...
  return <main className="page-shell"><div className="relative z-10 px-6 py-10">
    ...
    <div className="mt-6 grid items-start gap-6 lg:grid-cols-[200px_1fr]"><AdminSidebar/><section className="min-w-0 space-y-4">
      ...
      <form className="flex flex-wrap gap-2" onSubmit={(event) => { event.preventDefault(); void load() }}><input aria-label="Search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search records or Stripe IDs…" className="min-w-[220px] flex-1 rounded-2xl border border-[#dcdcdc] bg-white px-4 py-2 text-sm"/><input aria-label="Workspace filter" value={workspaceId} onChange={(e)=>setWorkspaceId(e.target.value)} placeholder="Workspace ID" className="min-w-[220px] rounded-2xl border border-[#dcdcdc] bg-white px-4 py-2 text-sm"/><input aria-label="Status filter" value={status} onChange={(e)=>setStatus(e.target.value)} placeholder="Status filter" className="rounded-2xl border border-[#dcdcdc] bg-white px-4 py-2 text-sm"/><button className="rounded-2xl bg-[#191919] px-4 py-2 text-sm font-semibold text-white">Refresh</button></form>
