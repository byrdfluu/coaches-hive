  ...
  return <main className="page-shell public-page"><div className="relative z-10 mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16">
    ...
    <section className="mt-12 grid gap-5 lg:grid-cols-2 xl:grid-cols-4">{plans.map(plan => {
      ...
      return <article key={plan.key} className={`relative flex flex-col rounded-3xl bg-white p-6 shadow-sm ${'popular' in plan && plan.popular ? 'border-2 border-[#b80f0a]' : 'border border-[#191919]'}`}>
        ...
        <div className="mt-auto pt-7"><Link href={'custom' in plan && plan.custom ? '/contact?topic=enterprise' : `/open-app?reason=web_signup_paused&from=${encodeURIComponent(`/plans/${plan.key}?billing_interval=${interval}`)}`} className={`inline-flex w-full justify-center rounded-full px-5 py-3 text-sm font-semibold text-white ${'popular' in plan && plan.popular ? 'bg-[#b80f0a]' : 'bg-[#191919]'}`}>{'custom' in plan && plan.custom ? 'Contact Sales' : 'Get the App'}</Link></div>
      </article>
    })}</section>
