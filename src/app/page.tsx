    ...
    <div className="relative z-10 mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      <section className="glass-card card-hero card-accent relative mt-8 overflow-hidden bg-white p-5 sm:mt-12 sm:p-8 lg:p-10">
        ...
        <div className="grid items-center gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-10">
          <div className="animate-rise min-w-0 space-y-6"><p className="text-xs font-bold uppercase tracking-[.3em] text-[#b80f0a]">One place instead of five</p><h1 className="display break-words text-[2.45rem] font-semibold leading-[1.04] text-[#1f1c18] sm:text-[3.7rem]" data-testid="hero-title">Stop running your program through group chats, spreadsheets, and payment apps.</h1><p className="max-w-xl text-lg leading-relaxed text-[#666] sm:text-xl">Coaches Hive brings schedules, messages, rosters, registrations, payments, and waivers together, so coaches can coach, parents know where to go, and organizations can see the whole operation.</p><div className="flex flex-wrap gap-3"><Link href="/signup" className="inline-flex items-center rounded-full bg-[#b80f0a] px-5 py-2.5 text-sm font-semibold text-white">Start your free trial</Link><Link href="#how-it-works" className="inline-flex items-center rounded-full border border-[#191919] bg-white px-5 py-2.5 text-sm font-semibold text-[#191919]">See how it works</Link></div><div className="flex flex-wrap gap-2">{['AAU','Travel sports','Club sports','Youth leagues'].map(label => <span key={label} className="rounded-full border bg-[#f7f7f7] px-3 py-1 text-xs font-semibold text-[#555]">{label}</span>)}</div></div>
          <div className="flex min-w-0 justify-center overflow-visible">
            <Image src="/home/coaches-hive-connected-dashboard.png" alt="Scattered texts, spreadsheets, payment apps, calendars, and paper waivers connected through the Coaches Hive organization dashboard and team mobile app" width={1400} height={1240} priority sizes="(max-width: 1023px) 100vw, 52vw" className="h-auto w-[108%] max-w-[756px] shrink-0 drop-shadow-[0_24px_45px_rgba(25,25,25,0.10)]" />
          </div>
        </div><SportsTicker />
      </section>
