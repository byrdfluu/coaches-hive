...
export default function AthleteTrainingPlansPage() {
  ...
  const load = useCallback(async () => { if (!activeSubProfileId) return; const response = await fetch(`/api/training-plans?athlete_profile_id=${encodeURIComponent(activeSubProfileId)}`, { cache: 'no-store' }); const data = await response.json().catch(() => ({})); if (!response.ok) setNotice(data.error || 'Unable to load plans.'); else setPlans(data.plans || []) }, [activeSubProfileId])
  useEffect(() => { void load() }, [load])
  const update = async (id: string, progress: string) => { setPlans((rows) => rows.map((row) => row.id === id ? { ...row, progress } : row)); const response = await fetch('/api/training-plans', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, progress, athlete_profile_id: activeSubProfileId }) }); if (!response.ok) { setNotice('Progress was not saved.'); await load() } }
  return <main className="page-shell"><div className="relative z-10 px-4 py-6 sm:px-6 sm:py-10"><div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]"><AthleteSidebar /><section>
    <p className="text-xs uppercase tracking-[0.3em] text-[#b80f0a]">Parent & Athlete Portal</p><h1 className="display text-3xl font-semibold">Training plans for {activeAthleteLabel}</h1><p className="mt-2 text-sm text-[#4a4a4a]">Plans assigned by coaches. Progress syncs with the mobile app.</p>
