...
export async function GET() {
  ...
  const items = [...(feed.data || []), ...extra].map((r: any) => ({ ...r, resolution: resolutionMap.get(r.event_id) || null })).sort((a, b) => Date.parse(b.occurred_at) - Date.parse(a.occurred_at))
  return NextResponse.json({ items: await enrichWithWorkspace(items), summary: { open: items.filter(i => i.resolution?.status !== 'resolved').length, resolved: items.filter(i => i.resolution?.status === 'resolved').length } })
}
...
export async function POST(request: Request) {
  ...
  const supabase = await createRouteHandlerClientCompat()
  const { error } = await supabase.rpc('admin_set_ops_issue_resolution', {
    p_issue_key: String(body.issue_key), p_title: String(body.title), p_detail: String(body.detail || ''),
    p_category: String(body.category || 'operations'), p_resolved: body.resolved !== false, p_note: String(body.note || ''),
  })
