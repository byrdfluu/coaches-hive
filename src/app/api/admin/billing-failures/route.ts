...
export async function GET(request:Request){
  ...
  return NextResponse.json({items,summary:{failures:items.length,subscriptions:items.filter(r=>r.source==='subscription').length,checkouts:items.filter(r=>r.source==='checkout').length,webhooks:items.filter(r=>r.source==='stripe_webhook').length}})
}
