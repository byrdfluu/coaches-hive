...
export async function GET(request: Request) {
  ...
  const { searchParams } = new URL(request.url); const query = searchParams.get('query')?.trim().toLowerCase() || ''; const status = searchParams.get('status') || ''
  let db = supabaseAdmin.from('stripe_connect_payment_accounting').select('*').order('created_at',{ascending:false}).limit(250)
  if (status) db = db.eq('checkout_type', status)
  ...
  const items = (data || []).filter((r:any)=>!query || JSON.stringify(r).toLowerCase().includes(query))
