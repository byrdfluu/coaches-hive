...
export async function GET(request: Request) {
  ...
  const enriched=await enrichWithWorkspace(data||[]),lower=query.toLowerCase(),items=enriched.filter((r:any)=>!query||JSON.stringify(r).toLowerCase().includes(lower)||resolved?.has(r.workspace_id))
