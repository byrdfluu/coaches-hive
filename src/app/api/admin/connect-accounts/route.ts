...
export async function GET(request: Request) {
  ...
  const matched=enriched.filter((r:any)=>!q||JSON.stringify(r).toLowerCase().includes(lower)||resolved?.has(r.workspace_id)).map((r:any)=>({...r,connect_ready:Boolean(r.charges_enabled&&r.payouts_enabled),requirements_due:(r.requirements_due||[]).join(', ')})),items=await filterAdminTestRows(matched,shouldShowTestData(p))
