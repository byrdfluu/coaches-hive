...
export async function GET(request: Request) {
  ...
  const orgId = await resolveActiveOrganizationId(session.user.id)
  if (!orgId) return jsonError('Organization not found', 404)
  const url = new URL(request.url); let query = supabaseAdmin.from('payment_transactions').select('*').eq('org_id', orgId).order('occurred_at', { ascending: false }).limit(5000)
  ...
  const { data: installments } = await supabaseAdmin.from('org_dues_installments').select('amount_due_cents,amount_paid_cents,due_at,status,org_dues_schedules!inner(org_id)').eq('org_dues_schedules.org_id', orgId)
