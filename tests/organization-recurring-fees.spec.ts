...
test('recurring fee checkout is bearer-authenticated and server-authorized', () => {
  ...
  expect(route).toContain("organization_recurring_fee_offer_assignments")
...
test('billing portal is limited to the payer and uses a dedicated configuration', () => {
  ...
  expect(route).not.toContain('isSuperadminUser(user)')
  expect(route).toContain('STRIPE_RECURRING_FEES_PORTAL_CONFIGURATION_ID')
...
test('payer accepts an authoritative assigned offer and cannot set commercial terms', () => {
  const route = read('src/app/api/mobile/recurring-fees/start/route.ts')
...
test('start to status lifecycle uses the authoritative fee id and reuses it idempotently', () => {
  const start = read('src/app/api/mobile/recurring-fees/start/route.ts')
  const status = read('src/app/api/mobile/recurring-fees/status/route.ts')
  expect(start).toContain('fee_id: fee.id')
  expect(start).toContain('fee_id: existing.id')
  expect(start).toContain(".eq('payer_user_id', user.id).eq('idempotency_key', idempotencyKey)")
  expect(start).toContain('authorizeRecurringFeePayer(user.id, existing.athlete_id, existing.organization_id)')
  expect(start).toContain("existing.offer_id !== offerId")
